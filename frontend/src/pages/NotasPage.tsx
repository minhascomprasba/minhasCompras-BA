        ...
        <>
          {data.data.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
              <p style={{ fontSize: '1.2rem', color: 'var(--text-primary)', marginBottom: '1rem' }}>Nenhuma nota encontrada no período.</p>
              <Link to="/importar" className="btn btn-secondary">Importar primeira nota</Link>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {data.data.map((nota) => (
                <div key={nota.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'transform 0.2s', padding: '1.5rem' }}>
                  <div>
                    <h3 style={{ color: 'var(--text-primary)', marginBottom: '0.25rem' }}>Nota #{nota.id}</h3>
